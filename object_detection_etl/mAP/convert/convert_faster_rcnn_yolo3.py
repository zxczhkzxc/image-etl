...
with open(annotation_file, 'r') as annot_f:
    for annot in annot_f:
        ...
        if ARGS.gen_recursive:
            ...
            file_name = img_path.replace('.png', '.txt').replace('/', '__')
        ...
        with open(output_file_path, 'w') as out_f:
            for bbox in annot[1:]:
                if ARGS.gt:
                    ...
                    x_min, y_min, x_max, y_max, class_name, score = list(map(str, bbox.split(',')))
                    # out_box = '{} {} {} {} {} {}'.format(
                    #     class_map[int(class_id)].strip(), score,  x_min, y_min, x_max, y_max)
                    out_box = '{} {} {} {} {} {}'.format(
                        class_name, "%.2f" %float(score), float(x_min), float(y_min), float(x_max), float(y_max))
